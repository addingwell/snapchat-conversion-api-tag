...
const events_api_url = 'https://tr.snapchat.com/' + api_version + '/' + data.pixelId + '/events' + (data.testEvents ? '/validate' : '') + '?access_token=' + encodeUriComponent(data.accessToken);
...
function getUserDataParameters() {
  ...
  if(user_data) {
    if(user_data.gender) {
      sc_user_data.ge = sha256Sync(user_data.gender.trim().charAt(0).toLowerCase(), { outputEncoding: "hex" });    
    } else if (user_data.sha256_gender) {
      sc_user_data.ge = user_data.sha256_gender;
    }
    
    if(user_data.email_address) {
      sc_user_data.em = sha256Sync(user_data.email_address.trim().toLowerCase(), { outputEncoding: "hex" });
    } else if(user_data.email) {
      sc_user_data.em = sha256Sync(user_data.email.trim().toLowerCase(), { outputEncoding: "hex" });
    } else if(user_data.sha256_email_address) {
      sc_user_data.em = user_data.sha256_email_address;
    }
    
    if(user_data.phone_number) { 
      sc_user_data.pn = sha256Sync(makeString(user_data.phone_number).replace(createRegex('[+ ]+', 'g'), ''), { outputEncoding: "hex" });
    } else if(user_data.sha256_phone_number) {
      sc_user_data.pn = user_data.sha256_phone_number;
    }
    
    if(getType(address) == 'object') {
      
      if(address.first_name) {
        sc_user_data.fn = sha256Sync(address.first_name.toLowerCase(), { outputEncoding: "hex" });
      } else if(address.sha256_first_name) {
        sc_user_data.fn = address.sha256_first_name;
      }
      
      if(address.last_name) {
        sc_user_data.ln = sha256Sync(address.last_name.toLowerCase(), { outputEncoding: "hex" });
      } else if(address.sha256_last_name) {
        sc_user_data.ln = address.sha256_last_name;
      }

      if(address.city) {
        sc_user_data.ct = sha256Sync(address.city.trim().toLowerCase(), { outputEncoding: "hex" });
      } else if(address.sha256_city) {
        sc_user_data.ct = address.sha256_city;
      }
      
      if(address.postal_code) {
        sc_user_data.zp = sha256Sync(address.postal_code.trim(), { outputEncoding: "hex" });
      } else if(address.sha256_postal_code) {
        sc_user_data.zp = address.sha256_postal_code;
      }
      
      if(address.country) {
        sc_user_data.country = sha256Sync(address.country.trim().toLowerCase(), { outputEncoding: "hex" });
      } else if(address.sha256_country) {
        sc_user_data.country = address.sha256_country;
      }
      
      if(address.state) {
        sc_user_data.st = sha256Sync(address.state.trim().toLowerCase(), { outputEncoding: "hex" });
      } else if(address.sha256_state) {
        sc_user_data.st = address.sha256_state;
      }
  ...
  if(getType(address) == 'array') {
    ...
    if(address[0].first_name) {
        sc_user_data.fn = sha256Sync(address[0].first_name.toLowerCase(), { outputEncoding: "hex" });
    }
    
    if(address[0].last_name) {
       sc_user_data.ln = sha256Sync(address[0].last_name.toLowerCase(), { outputEncoding: "hex" });
    }
    
    if(address[0].city != undefined) {
        sc_user_data.ct = sha256Sync(address[0].city.trim().toLowerCase(), { outputEncoding: "hex" });
    }
    
    if(address[0].postal_code) {
        sc_user_data.zp = sha256Sync(address[0].postal_code.trim(), { outputEncoding: "hex" });
    }
    
    if(address[0].state) {
        sc_user_data.st = sha256Sync(address[0].state.trim().toLowerCase(), { outputEncoding: "hex" });
    }
    
    if(address[0].country) {
        sc_user_data.country = sha256Sync(address[0].country.trim().toLowerCase(), { outputEncoding: "hex" });
     }
  ...
  if (data.userDataList) {
    data.userDataList.forEach(d => {
      if(d.name == "pn") {
        sc_user_data[d.name] = sha256Sync(d.value.replace(createRegex('[+ ]+', 'g'), ''), { outputEncoding: "hex" });
      } else {
        sc_user_data[d.name] = sha256Sync(d.value.trim().toLowerCase(), { outputEncoding: "hex" });             
      }
    });
  }
